...
/*
    ...
    UPDATES:
    - Added `core` role.
    - `setCore` can be called once by owner.
    - All trade execution functions are restricted to `onlyCore`.
*/
...
contract BrokexVault {
    ...
    address public core;      // NEW: The Core contract address
    bool public coreSet;      // NEW: Flag to ensure core is set only once
    ...
    // NEW: Restrict access to the Core address
    modifier onlyCore() {
    ...
    event CoreSet(address indexed core); // NEW
    ...
    function createOrder(
        ...
        trades[tradeId] = Trade({
            id: tradeId,
            owner: trader, // Trader passed in arg is the owner
            margin: margin6,
    ...
    function cancelOrder(uint256 tradeId) external onlyCore {
        ...
        // Auth check removed here because Core is trusted to validate request
