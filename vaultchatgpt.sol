...
contract BrokexVault {
    ...
    */
    ...
    /// @notice Retourne les shares (LP tokens) obtenues par `lp` à partir du dépôt de l'epoch `e`.
    /// @dev Si l'epoch n'est pas clôturée (price=0), renvoie 0.
    function getLpSharesForEpoch(address lp, uint256 e) external view returns (uint256 shares18) {
        uint256 dep6 = pendingDepositOf[lp][e];
        if (dep6 == 0) return 0;

        uint256 price = lpTokenPrice[e];
        if (price == 0) return 0; // epoch pas clôturée
