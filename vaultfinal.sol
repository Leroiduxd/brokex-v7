...
/*
    ...
    Units:
      - USD amounts: 6 decimals (USDC-like)
      - LP shares:   18 decimals (WAD)

    Fees:
      - Commission at open: 30% -> owner (freeBalance[owner]), 70% -> LP (lpFreeCapital)
      - Trader profit fee: 1% of PROFIT ONLY (when pnl > 0) -> LP (lpFreeCapital)
        (when trader loses: no extra fee)

    Dust:
      - If total LP capital (lpFree + lpLocked) is < 5 USD (5e6), we hard-reset:
          - sweep remaining capital to owner freeBalance
          - set lpFreeCapital, lpLockedCapital, totalShares to 0
          - ignore unrealizedPnL for that roll
      - A public function `sweepDust()` is also provided.

    NOTE (requested):
      - For opening an order/position, the trade owner is passed as an argument (not msg.sender).
      - Safety guard: only the trader themselves or the contract owner can act on behalf of the trader.

    NEW (requested): "LP withdraw reserve" to prevent withdrawals becoming infinite
      - withdrawSharesUnfunded18: shares requested for withdrawal but NOT YET funded (NOT yet burned)
        * increases on requestLpWithdrawFromEpochs()
        * decreases at rollEpoch() exactly when we burn shares (fund them)
      - minLpFreeReserve6: USD amount that must remain free to prioritize pending withdrawals
        * updated once per rollEpoch, using the roll price:
            minLpFreeReserve6 = withdrawSharesUnfunded18 * priceWad (-> 18) then converted to 6 decimals
      - _lpLock() is updated to require:
            lpFreeCapital >= minLpFreeReserve6 + amount6
        so new trades cannot consume liquidity needed to fund the withdrawal queue.

    IMPORTANT: No other logic changed.
*/
...
contract BrokexVault {
    ...
    // -----------------------------
    uint256 public lpFreeCapital;    // 6 decimals
    uint256 public lpLockedCapital;  // 6 decimals
    ...
    struct Trade {
        uint256 id;
        address owner;        // trader address (passed as argument)
        uint256 margin;       // 6 decimals
        uint256 commission;   // 6 decimals
        uint256 lpLock;       // 6 decimals
        TradeState state;
    }
    ...
    mapping(uint256 => int256)  public epochEquitySnapshot18; // epoch -> equity18
    ...
    mapping(uint256 => WithdrawBucket) public withdrawBuckets; // requestEpoch => bucket
    mapping(uint256 => mapping(address => UserWithdraw)) public userWithdraws; // requestEpoch => user => data
    ...
    struct PayoutTranche {
        uint256 sharesRemaining18; // paid shares to be assigned to buckets
        uint256 priceWad;          // price used for those paid shares
    }

    mapping(uint256 => PayoutTranche) public payoutByEpoch; // payEpoch => tranche
    ...
    // -----------------------------
    // NEW: Withdraw reserve guard (requested)
    // -----------------------------
    // Shares requested but NOT YET FUNDED (not yet burned). Decrease at burn time.
    uint256 public withdrawSharesUnfunded18;

    // USD reserve target (6 decimals) derived from withdrawSharesUnfunded18 * last roll price
    ...
    event WithdrawRequested(
        address indexed lp,
        uint256 indexed requestEpoch,
        uint256 sharesAdded18,
        uint256 newUserShares18,
        uint256 newBucketTotalShares18
    );

    event WithdrawClaimed(address indexed lp, uint256 indexed requestEpoch, uint256 amount6);

    event PayoutCreated(
        uint256 indexed payEpoch,
        uint256 sharesPaid18,
        uint256 usdReserved6,
        uint256 priceWad
    );

    event PayoutAssignedToBucket(
        uint256 indexed payEpoch,
        uint256 indexed bucketEpoch,
        uint256 sharesAssigned18,
        uint256 usdAllocated6
    );
    ...
    // Full precision mulDiv (OpenZeppelin-style)
    ...
    function sweepDust() public {
        ...
        // Send remaining tiny capital to owner balance and hard reset
        freeBalance[owner] += cap6;
        lpFreeCapital = 0;
        lpLockedCapital = 0;
        totalShares = 0;

        // Keep reserve system coherent after a hard reset
    ...
    function _lpLock(uint256 amount6) internal {
        // NEW (requested): keep lpFreeCapital >= minLpFreeReserve6 after opening new locks
    ...
    // Split commission: 30% to owner, 70% to LP
    ...
    // pnl is signed WAD in "USD units"
    // Profit fee: 1% of PROFIT ONLY goes to LP (reduces what trader receives).
    ...
    function cancelOrder(uint256 tradeId) external {
        ...
        // owner of trade OR contract owner can cancel
        require(msg.sender == t.owner || msg.sender == owner, "not allowed");

        t.state = TradeState.Cancelled;

        // Refund margin + commission back to trader locked -> free
    ...
    // -----------------------------
    // LP: withdrawal request (select deposit epochs -> shares -> bucket[currentEpoch])
    // -----------------------------
    function requestLpWithdrawFromEpochs(uint256[] calldata depositEpochs) external {
        ...
        for (uint256 i = 0; i < depositEpochs.length; i++) {
            ...
            pendingDepositOf[msg.sender][e] = 0; // remove from deposit-ledger
        ...
        // NEW (requested): these shares are unfunded until burned in rollEpoch
        ...
        emit WithdrawRequested(
            ...
            sharesToAdd18,
            u.sharesRequested18,
            b.totalSharesInitial18
        );
    }

    // -----------------------------
    // Epoch rollover (Option A) + create payout tranche (burn what can be paid)
    // -----------------------------
    ...
    function rollEpoch(int256 unrealizedPnlTraders18) external {
        ...
        // ---- DUST HARD RESET (before pricing) ----
        ...
        // If shares are 0, we restart at $1/share.
        if (totalShares == 0) {
        ...
        // Mint global shares for deposits in epoch e
        ...
        // ---- Withdrawal payout tranche creation (burn only what cash can pay) ----
        ...
        if (unpaidMinusPaid18 > 0 && lpFreeCapital > 0) {
            ...
            // NEW (requested): only shares still unfunded should affect reserve; decrease at burn time
            ...
            if (payShares18 > 0) {
                ...
                // decrease unfunded shares exactly when we burn (fund them)
        ...
        // NEW (requested): update reserve target once per epoch using the last roll price
        // minLpFreeReserve6 = withdrawSharesUnfunded18 * priceWad (-> WAD) then convert to 6 decimals
    ...
    // -----------------------------
    // Assign paid shares FIFO into withdrawal buckets (no user loops)
    // -----------------------------
    ...
    // -----------------------------
    // Views: withdraw frontend helpers
    // -----------------------------
